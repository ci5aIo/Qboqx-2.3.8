...
$view_type = elgg_extract('list_type', $vars, false);
if ($input_name){
    $input_tag = elgg_view('input/hidden', array('name'=>$input_name, 'value'=>$file->$input_value_field));
}
...
if ($full_view && !elgg_in_context("gallery")) {
	...
	<div class='file-gallery-item'>
	    <h3>" . $file->title . "</h3>".
	    elgg_view_entity_icon($file, "medium").
	    "<p class='elgg-quiet'>$owner_link $date</p>
	</div>";
} 
elseif($view_type == 'list_experience'){
    // experience listing view of the file
	$file_icon_alt = "";
