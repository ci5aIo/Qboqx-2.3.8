<?php
	...
	$folder_subtype_id = get_subtype_id("object", item_tools_SUBTYPE);
	$parent_guid_id = elgg_get_metastring_id("parent_guid");
	$zero_metadata_id = elgg_get_metastring_id(0);
	...
	if (!empty($folder_subtype_id)) {
		...
		$query = "CREATE TABLE item_tools_fix";
		...
		$query .= " WHERE id IN (SELECT id FROM item_tools_fix)";
		
		// execute the update query
		update_data($query);
		
		// cleanup the temp table
		$query = "DROP TABLE item_tools_fix";
