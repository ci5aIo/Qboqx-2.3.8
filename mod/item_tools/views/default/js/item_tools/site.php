...
elgg.item_tools.uploadify.init = function() {
	...
	if ($uploadifyButton.length) {
		...
		$uploadifyButton.uploadify({
			swf: elgg.normalize_url("mod/item_tools/vendors/uploadify/uploadify.swf"),
			uploader: elgg.normalize_url("mod/item_tools/procedures/upload/multi.php"),
...
elgg.item_tools.tree.init = function() {
	...
	if ($tree.length) {
		var item_tools_old_parent_guid;
		...
		$tree.tree({
			...
			callback: {
				...
				beforemove: function(node, ref_node, type, tree_obj) {
					item_tools_old_parent_guid = tree_obj.parent(node).find('a:first').attr('href').substr(1);
				...
				onmove: function(node, ref_node, type, tree_obj, rb) {
					...
					if ((parent_guid == window.location.hash.substr(1)) || (item_tools_old_parent_guid == window.location.hash.substr(1))) {
						if (parent_guid == window.location.hash.substr(1)) {
							refresh = parent_guid;
						} else if (item_tools_old_parent_guid == window.location.hash.substr(1)) {
							refresh = item_tools_old_parent_guid;
						}
					...
					elgg.action("item_tools/folder/reorder", {
						...
						success: function() {
							if (refresh !== false) {
								elgg.item_tools.load_folder(refresh);
							}
						}
					});
				}
...
elgg.item_tools.show_more_files = function() {
	$(this).hide();
	$('#item_tools_list_items div.elgg-ajax-loader').show();
	...
	elgg.post("item_tools/list/" + elgg.get_page_owner_guid(), {
		data: {
			folder_guid: folder,
		...
		success: function(data) {
			...
			$('#item_tools_list_items ul.elgg-list').append(li);
			elgg.item_tools.initialize_file_draggable();

			// replace the show more button with new data
			var show_more = $(data).find("#item-tools-show-more-wrapper");
			$("#item-tools-show-more-wrapper").replaceWith(show_more);

			// hide ajax loader
			$('#item_tools_list_items div.elgg-ajax-loader').hide();
		}
	});
	 
}

elgg.item_tools.initialize_file_draggable = function() {
	
	$("#item_tools_list_items .item-tools-item").draggable({
		revert: "invalid",
		opacity: 0.8,
		appendTo: "body",
		helper: "clone",
...
elgg.item_tools.initialize_folder_droppable = function() {
	$("#item_tools_list_items .item-tools-folder").droppable({
		accept: "#item_tools_list_items .item-tools-item",
		drop: function(event, ui){
			droppable = $(this);
			draggable = ui.draggable;
...
elgg.item_tools.init = function() {
	...
	$('#item-tools-show-more-files').live("click", elgg.item_tools.show_more_files);
