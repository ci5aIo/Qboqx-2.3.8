...
if ($user_time_preference = elgg_get_plugin_user_setting("item_tools_time_display", null, "item_tools")) {
	$time_preference = $user_time_preference;
} elseif ($site_time_preference = elgg_get_plugin_setting("item_tools_default_time_display", "item_tools")) {
...
if ($full_view && !elgg_in_context("gallery")) {
	...
	if (item_tools_use_folder_structure()) {
		// @todo this should probably be moved to the file view page, but that is currently not under control of item_tools
...
//			'relationship' => 'folder_of',
	...
	$file_icon_alt = "";
	if (item_tools_use_folder_structure()) {
		$file_icon = elgg_view_entity_icon($file, "tiny", array("img_class" => "item-tools-icon-tiny"));
		
		if (elgg_in_context("item_tools_selector")) {
	...
	echo elgg_view_image_block($file_icon, $list_body, array("class" => "item-tools-item", "image_alt" => $file_icon_alt));
}
