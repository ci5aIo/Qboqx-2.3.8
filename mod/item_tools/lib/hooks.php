 ...
 * @return bool
 */
function item_tools_can_edit_metadata_hook($hook, $type, $returnvalue, $params) {
	...
	if (empty($entity) || !elgg_instanceof($entity, "object", item_tools_SUBTYPE)) {
		return $returnvalue;
	}
	...
	if ($container_entity->isMember($user) && ($container_entity->item_tools_structure_management_enable != "no")) {
		// is group member
		$returnvalue = true;
	}
 ...
 * @return string
 */
function item_tools_folder_icon_hook($hook, $type, $returnvalue, $params) {
	...
	switch($size){
		...
		case "tiny":
		case "small":
			$returnvalue = "mod/item_tools/_graphics/folder/" . $size . ".png";
			break;
		default:
			$returnvalue = "mod/item_tools/_graphics/folder/medium.png";
			break;
	}
 ...
 * @return array
 */
function item_tools_write_acl_plugin_hook($hook, $type, $returnvalue, $params) {
	
	if (empty($params) || !is_array($params)) {
		return $returnvalue;
	}

	if (!elgg_in_context("item_tools")) {
		return $returnvalue;
	}
 ...
 * @return array|bool
 */
function item_tools_file_route_hook($hook, $type, $returnvalue, $params) {
	...
	switch ($page[0]) {
		case "view":
			...
			break;
		case "owner":
			if (item_tools_use_folder_structure()) {
				$returnvalue = false;
 ...
 * @return string
 */
function item_tools_widget_url_hook($hook, $type, $returnvalue, $params) {
 ...
 * @return ElggMenuItem[]
 */
function item_tools_folder_sidebar_tree_hook($hook, $type, $returnvalue, $params) {
	...
	if ($folders = item_tools_get_folders($container->getGUID())) {
		$main_menu_item->setChildren(item_tools_make_menu_items($folders));
	}
 ...
 * @return ElggMenuItem[]
 */
function item_tools_entity_menu_hook($hook, $type, $returnvalue, $params) {
	...
	if (elgg_instanceof($entity, "object", item_tools_SUBTYPE)) {
 ...
 * @return string
 */
function item_tools_folder_url_handler($hook, $type, $returnvalue, $params) {
	...
	if (elgg_instanceof($container, "group")) {
		$returnvalue = "item/group/" . $container->getGUID() . "/all#" . $entity->getGUID();
	} else {
		$returnvalue = "item/owner/" . $container->username . "#" . $entity->getGUID();
	}
