 ...
 * @return string
 */
function item_tools_get_file_extension($file) {
	$result = '';
 ...
 * @return array
 */
function item_tools_get_zip_structure($folder, $prepend) {
	$entries = array();
	...
	if ($entities = elgg_get_entities_from_metadata($options)) {
		foreach ($entities as $subfolder) {
			...
			$entries[] = array('directory' => $title, 'files' => item_tools_has_files($subfolder->getGUID()));
			
			$entries = array_merge($entries, item_tools_get_zip_structure($subfolder, $title));
		}
	}
 ...
 * @return bool|array
 */
function item_tools_has_files($folder) {
	$files_options = array(
		"type" => "object",
		"subtype" => item_tools_item_SUBTYPE,
		"limit" => false,
		//"container_guid" => get_loggedin_userid(),
		"relationship" => item_tools_RELATIONSHIP,
 ...
 */
function item_tools_get_folders($container_guid = 0) {
	$result = false;
	...
	if (!empty($container_guid)) {
		...
		if ($folders = elgg_get_entities($options)) {
			...
			foreach ($folders as $folder) {
				...
				if (!empty($parent_guid)) {
					if ($temp = get_entity($parent_guid)) {
						if ($temp->getSubtype() != item_tools_SUBTYPE) {
							$parent_guid = 0;
						}
					} else {
						$parent_guid = 0;
			...
			$result = item_tools_sort_folders($parents, 0);
		}
	}
 ...
 * @return string
 */
function item_tools_build_select_options($folders, $depth = 0) {
	$result = array();
	...
	if (!empty($folders)) {
		foreach ($folders as $index => $level) {
			...
			if ($childen = elgg_extract("children", $level)) {
				$result += item_tools_build_select_options($childen, $depth + 1);
			}
		}
	}
 ...
 * @return string
 */
function item_tools_build_widget_options($folder, $internalname = "", $selected = array()) {
	$result = "";
	
	if (is_array($folder) && !array_key_exists("children", $folder)) {
		foreach ($folder as $folder_item) {
			$result .= "<ul>";
			$result .= item_tools_build_widget_options($folder_item, $internalname, $selected);
			$result .= "</ul>";
		}
	} else {
		...
		if (!empty($folder["children"])) {
			$result .= item_tools_build_widget_options($folder["children"], $internalname, $selected);
		}
		$result .= "</li>";
	}
 ...
 * @return bool|array
 */
function item_tools_sort_folders($folders, $parent_guid = 0) {
	$result = false;
	...
	if (array_key_exists($parent_guid, $folders)) {
		...
		foreach ($folders[$parent_guid] as $subfolder) {
			$children = item_tools_sort_folders($folders, $subfolder->getGUID());
 ...
 */
function item_tools_get_sub_folders($folder = false, $list = false) {
	$result = false;
 ...
 */
function item_tools_make_menu_items($folders) {
	$result = false;
		...
		foreach ($folders as $index => $level) {
			if ($folder = elgg_extract("folder", $level)) {
				...
				if ($children = elgg_extract("children", $level)) {
					$folder_menu->setChildren(item_tools_make_menu_items($children));
				}
 ...
 * @return void
 */
function item_tools_change_children_access($folder, $change_files = false) {
	
	if (!empty($folder) && ($folder instanceof ElggObject)) {
		if ($folder->getSubtype() == item_tools_SUBTYPE) {
			...
			if ($children = elgg_get_entities_from_metadata($options)) {
				foreach ($children as $child) {
					...
					item_tools_change_children_access($child, $change_files);
				}
			}
			
			if ($change_files) {
				// change access on files in this folder
				item_tools_change_files_access($folder);
			}
		}
	}
}
 ...
 * @return void
 */
function item_tools_change_files_access($folder) {
	...
	if (!empty($folder) && ($folder instanceof ElggObject)) {
		if ($folder->getSubtype() == item_tools_SUBTYPE) {
			...
			if ($files = elgg_get_entities_from_relationship($options)) {
				...
				elgg_unregister_event_handler("update", "object", "item_tools_object_handler");
 ...
 * @return bool|string
 */
function item_tools_allowed_extensions($zip = false) {
	
	$allowed_extensions_settings = elgg_get_plugin_setting('allowed_extensions', 'item_tools');
 ...
 * @return bool|ElggObject
 */
function item_tools_check_foldertitle_exists($title, $container_guid, $parent_guid = 0) {
	$result = false;
 ...
 * @return void
 */
function item_tools_create_folders($zip_entry, $container_guid, $parent_guid = 0) {
	...
	if (!empty($zdir)) {
		...
		if (count($sub_folders) == 1) {
			$entity = item_tools_check_foldertitle_exists($zdir, $container_guid, $parent_guid);

			if (!$entity) {
				$directory = new ElggObject();
				$directory->subtype = item_tools_SUBTYPE;
			...
			foreach ($sub_folders as $folder) {
				if ($entity = item_tools_check_foldertitle_exists($folder, $container_guid, $parent)) {
 ...
 * @return bool
 */
function item_tools_unzip($file, $container_guid, $parent_guid = 0) {
	$extracted = false;
	
	if (!empty($file) && !empty($container_guid)) {
		$allowed_extensions = item_tools_allowed_extensions();
		...
		if ($access_id === false) {
			if (!empty($parent_guid) && ($parent_folder = get_entity($parent_guid)) && elgg_instanceof($parent_folder, "object", item_tools_SUBTYPE)) {
		...
		while ($zip_entry = zip_read($zip)) {
			...
			if (strlen($zip_entry_name) != strlen($filename)) {
				...
				item_tools_create_folders($zip_entry, $container_guid, $parent_guid);
			}
				...
				if ($entity = item_tools_check_foldertitle_exists($folder, $container_guid, $parent)) {
					...
					if ($folder == end($folder_array)) {
						...
						if (in_array(strtolower($file_extension), $allowed_extensions)) {
							...
							if (!empty($parent)) {
								add_entity_relationship($parent, item_tools_RELATIONSHIP, $filehandler->getGUID());
							}
						}
					}
				}
 ...
 * @return bool
 */
function item_tools_use_folder_structure() {
	static $result;
	
	if (!isset($result)) {
		$result = false;
		
		if (elgg_get_plugin_setting("use_folder_structure", "item_tools") == "yes") {
			$result = true;
		}
	}
 ...
 * @return void
 */
function item_tools_add_folder_to_zip(ZipArchive &$zip_archive, ElggObject $folder, $folder_path = "") {
	
	if (!empty($zip_archive) && !empty($folder) && elgg_instanceof($folder, "object", item_tools_SUBTYPE)) {
		...
		if ($sub_folders = elgg_get_entities_from_metadata($folder_options)) {
			foreach ($sub_folders as $sub_folder) {
				item_tools_add_folder_to_zip($zip_archive, $sub_folder, $folder_path);
			}
		}
	}
}
 ...
 * @return string
 */
function item_tools_get_readable_file_size_limit() {
 ...
 * @return int
 */
function item_tools_get_list_length() {
	static $result;
	
	if (!isset($result)) {
		$result = 50;
		
		$setting = (int) elgg_get_plugin_setting("list_length", "item_tools");
		if ($setting < 0) {
			$result = false;
		} elseif ($setting > 0) {
			$result = $setting;
