 ...
 * @return void
 */
function item_tools_object_handler($event, $type, $object) {
	...
	if (!empty($object) && elgg_instanceof($object, "object", "file")) {
		...
		remove_entity_relationships($object->getGUID(), item_tools_RELATIONSHIP, true);
			
		if (!empty($folder_guid)) {
			add_entity_relationship($folder_guid, item_tools_RELATIONSHIP, $object->getGUID());
		}
	}
}
 ...
 * @return void
 */
function item_tools_object_handler_delete($event, $type, $object) {
	static $delete_files;
	
	if (!empty($object) && elgg_instanceof($object, "object", item_tools_SUBTYPE)) {
		// find subfolders
		$options = array(
			"type" => "object",
