...
if (!empty($folder_guid) && (!empty($parent_guid) || $parent_guid == 0)) {
	...
	if (!empty($parent_guid)) {
		if (!($parent = get_entity($parent_guid)) || !elgg_instanceof($parent, "object", item_tools_SUBTYPE)) {
			unset($parent_guid);
		}
	}
	...
	if (!is_null($parent_guid) && ($folder_guid != $parent_guid) && ($folder = get_entity($folder_guid))) {
		if (elgg_instanceof($folder, "object", item_tools_SUBTYPE) && $folder->canEditMetadata("parent_guid")) {
			// set new parent_guid
	...
	if (!empty($order) && !is_null($parent_guid)) {
		foreach ($order as $index => $order_guid) {
			if ($folder = get_entity($order_guid)) {
				if (elgg_instanceof($folder, "object", item_tools_SUBTYPE) && $folder->canEditMetadata("order")) {
	...
	system_message(elgg_echo("item_tools:action:folder:reorder:success"));
} else {
