...
$parent_guid = (int) get_input("item_tools_parent_guid", 0); // 0 is top_level
	...
	if (($owner = get_entity($owner_guid)) && (elgg_instanceof($owner, "user") || elgg_instanceof($owner, "group"))) {
		if (!empty($guid)) {
			...
			if ($folder = get_entity($guid)) {
				if (!elgg_instanceof($folder, "object", item_tools_SUBTYPE)) {
					unset($folder);
				}
			}
		} else {
			...
			$folder->subtype = item_tools_SUBTYPE;
		...
		if (!empty($folder)) {
			...
			if (($parent_guid === 0) || ($parent_guid != $folder->getGUID())) {
				...
				if (!empty($change_children_access)) {
					$folder->save();
					item_tools_change_children_access($folder, !empty($change_files_access));
				...
				if ($folder->save()) {
					...
					system_message(elgg_echo("item_tools:action:edit:success"));
				} else {
					register_error(elgg_echo("item_tools:action:edit:error:save"));
				}
			} else {
				register_error(elgg_echo("item_tools:action:edit:error:parent_guid"));
			}
		} else {
			register_error(elgg_echo("item_tools:action:edit:error:folder"));
		}
	} else {
		register_error(elgg_echo("item_tools:action:edit:error:owner"));
	}
} else {
	register_error(elgg_echo("item_tools:action:edit:error:input"));
}
