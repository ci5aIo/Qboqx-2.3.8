		...
		return {
			...
			attach_events : function () {
				...
				$("#" + this.container.attr("id") + " li a")
					.on("click.jstree", function (event) { // WHEN CLICK IS ON THE TEXT OR ICON
					...
					.on("dblclick.jstree", function (event) { // WHEN DOUBLECLICK ON TEXT OR ICON
						if(_this.locked) {
							event.preventDefault(); 
							event.stopPropagation();
							event.target.blur();
					...
					.on("contextmenu.jstree", function (event) {
						if(_this.locked) {
							event.target.blur();
							return _this.error("LOCKED");
						}
					...
					.on("mouseover.jstree", function (event) {
						if(_this.locked) {
							event.preventDefault();
							event.stopPropagation();
							return _this.error("LOCKED");
