...
$user   = $vars['user'];
$friend = $vars['entity'];
//$friend_guid = $vars['friend'];
$entity = $vars['entity'];
//$friend = get_entity($friend_guid);
/*
$friend = get_user_by_username($vars['friend']);
if (!($friend instanceof \ElggUser)) {
	echo elgg_echo('labels:invalid:user:guid');
	return;
}
*/
$friendcollections = 'this label';
$friendcollections .= labels_get_friend_collections_II($user, $friend);

$approve = $vars['approve'];

$body = '<label for="rtags">' . elgg_echo('labels:rtags') . '</label>';

$body .= elgg_view('input/text', array(
	'name' => 'rtags',
	'id' => 'labels_rtags',
	'value' => $friendcollections,
		));
...
$friendcollections = labels_get_friend_collections($user, $friend);
...
$body .= "<label>" . elgg_echo('labels:notifications:method', array($friend->name)) . "</label>";
