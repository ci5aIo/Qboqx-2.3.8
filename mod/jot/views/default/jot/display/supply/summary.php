...
$subtype      = $supply->getSubtype();
$fields       = jot_prepare_brief_view_vars($subtype, $supply, $section);
//echo elgg_dump($fields);
//$fields       = supply_prepare_brief_view_vars($supply, $section);
echo "<!--Section = $section-->";
/**/
$documents = elgg_get_entities_from_relationship(array(
	'type' => 'object',
	'relationship' => 'document',
	'relationship_guid' => $supply->guid,
...
$issues = elgg_get_entities_from_relationship(array(
	...
	foreach ($fields as $label => $value) {
	    ...
	    $section1 .= "<tr><td nowrap><b>".elgg_echo('supplys:'.$label)."<b></td><td style='padding-left: 15px;'>$value</td></tr>";
	}
	...
	echo '<table><tr><td width=100%><b>Documentation</b></td>
		      ...
		      elgg_view('output/url', array(
                ...
                'href' => "jot/attach?element_type=document&container_guid=" . $supply->guid)).'</td>';
	echo '</tr>
	      <tr><td colspan=2>';
	...
	echo '<tr><td><b>Issues</b></td>
	      ...
	      elgg_view('output/url', array(
			'text' => '[add new issue]', 
			'href' => "jot/add/$supply_guid/issue")).'</td>';
	echo '</tr>
	      <tr><td colspan=2>';
		if ($issues) {
		foreach ($issues as $i) {
	...
	echo '<tr><td><b>Tasks</b></td>
	      ...
	      elgg_view('output/url', array(
			'text' => '[add new task]', 
			'href' => "tasks/add/".$entity->owner_guid."?element_type=task&container_guid=".$supply->guid
	      )).'
	      </td></tr>';
