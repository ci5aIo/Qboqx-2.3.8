...
//$fields       = cause_prepare_brief_view_vars($cause);
// echo elgg_dump($vars);
// echo elgg_dump($fields);
// echo elgg_dump($cause);

/**/
$documents = elgg_get_entities_from_relationship(array(
	'type' => 'object',
	'relationship' => 'document',
	'relationship_guid' => $cause->guid,
...
$issues = elgg_get_entities_from_relationship(array(
	...
	foreach ($fields as $label => $value) {
		...
		if ($label   == 'Root'){
			$checkbox = elgg_view("input/checkbox", array("name" => $label, "value" => $value ));
			$section1 .= "<tr><td>$checkbox <b>$label<b></td><td style='padding-left: 15px;'>&nbsp;</td></tr>";
		}
	...
	echo '<table><tr><td width=100%><b>Documentation</b></td>
		      ...
		      elgg_view('output/url', array(
                ...
                'href' => "jot/attach?element_type=document&container_guid=" . $cause->guid)).'</td>';
	echo '</tr>
	      <tr><td colspan=2>';
	...
	echo '<tr><td><b>Issues</b></td>
	      ...
	      elgg_view('output/url', array(
			'text' => '[add new issue]', 
			'href' => "jot/add/$cause->guid/issue")).'</td>';
	echo '</tr>
	      <tr><td colspan=2>';
		if ($issues) {
		foreach ($issues as $i) {
			echo '<tr>';
				echo '<td class="highlight">'.elgg_view('output/url', array('text' => $i->title,'href' =>  'jot/view/'.$i->guid)).'</td>';
			echo '</tr>';
		}
	}	
	echo '<tr><td colspan=2>&nbsp;</td></tr>';
	echo '<tr><td><b>Tasks</b></td>
	      ...
	      elgg_view('output/url', array(
			'text' => '[add new task]', 
			'href' => "tasks/add/".$entity->owner_guid."?element_type=task&container_guid=".$cause->guid
	      )).'
	      </td></tr>';
