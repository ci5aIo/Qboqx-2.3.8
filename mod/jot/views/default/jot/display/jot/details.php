...
// discoveries
	echo '<table width = 100%>
	      ...
	      <tr>
	        <td colspan=2>'.
    elgg_view_form('jot/add/element', array("action" => "action/jot/add/element?element_type=observation&guid=$jot_guid&asset=$asset")).'
	        </td>
	      </tr>';
if ($observations) {
foreach ($observations as $i) {
			...
			if ($i->canEdit()) {
				$delete = elgg_view("output/url",array(
			    	'href' => "action/jot/delete?guid=$i->guid&container_guid=$jot_guid&tab=Details",
	...
	echo '<tr class="highlight">
	        <td>'.elgg_view('output/url', array('text' => $i->title,'href' =>  'jot/observation/'.$i->guid)).'</td>
	        <td>'.$delete.'
	      </tr>';
    }
}	
	echo '<tr>
	      ...
	      <tr>
	        <td colspan=2>'.
	elgg_view_form('market/add/element', array("action" => "action/jot/add/element?element_type=cause&guid=$jot_guid")).'
	        <td>
	      </tr>';
if ($causes) {
foreach ($causes as $i) {
...
$icon = elgg_view_icon('unlink');
	...
	elgg_view('output/url', array(
				            ...
				            'href' => "jot/attach?element_type=document&container_guid=" . $jot_guid)).'
            </td>
          <tr>';
if ($documents) {
foreach ($documents as $i) {
			...
			if ($i->canEdit()) {
				$detach = elgg_view("output/url",array(
			    	'href' => "action/jot/detach?element_type=$element_type&guid=$i->guid&container_guid=$jot_guid",
