...
$asset          = get_entity($asset_guid);                                 $display .='29 $vars[show_receive] = '.$vars['show_receive'].'<br>';
...
}                                                                     $display .= '82 $title: '.$title.'<br>';
                                                                      $display .= '83 $transfer_exists: '.$transfer_exists.'<br>76 $item_exists: '.$item_exists.'<br>';
$receipt_read_only = ($entity->status == 'R');                        // Disable all input when the jot has been received
...
if ($transfer_exists && elgg_entity_exists($transfer_guid)){ 
    ...
    $merchant = $transfer->merchant;                                                                          $display .= '132 $merchant: '.$merchant.'<br>';
    if (elgg_entity_exists($merchant) && is_int($merchant)/*accounts for merchants whose name begins with a number*/){
        if (isset($merchant) && elgg_entity_exists($merchant)){$merchants = array(get_entity($merchant));}     $display .= '134 isset($merchants): '.isset($merchants).'<br>113 $merchant: '.print_r($merchant, true).'<br>';
    ...
    }                                                                        $display .= '155 isset($merchants): '.isset($merchants).'<br>';
}
else {
...
}                                                                                 $display .= '166 $merchant: '.$merchant.'<br>';
                                                                                  $display .= '167 $receipt_items[0][retain_line_label] = '.$receipt_items[0]['retain_line_label'].'<br>167 $receipt_items->retain_line_label = '.$receipt_items->retain_line_label.'<br>';
...
elseif (elgg_instanceof(get_entity($merchant), 'group')){                                                       $display .= '236 $merchant: '.$merchant.'<br>';
    $merchant = elgg_view('output/span', ['content'=>elgg_view('input/grouppicker', array(
    		        ...
    		        'class'=>'receipt-input',]);}
else {                                                                                              $display .= '243 $merchant: '.$merchant.'<br>';
    $merchant = elgg_view('output/span', ['content'=>elgg_view('input/grouppicker', array(
							'name' => 'jot[merchant]',
		                    'value'  => $merchant,                  // singular - not an existing group
                            'limit'=> 1,
                            'autocomplete'=>'on',
					)),
    		        'class'=>'receipt-input',]);}
...
$merchant_value = elgg_view('output/span',['content'=>get_entity($transfer->merchant)->name,'class'=>'receive-output']);
...
foreach($hidden_fields as $name => $value){
    ...
    $preorder_label = 'Pre ordered';
    $preorder_options =                              ['name'=>'jot[preorder_flag]'            , 'value'=>$transfer->preorder_flag, 'class'=>'preorder-flag','label_class'=>'receipt-input', 'readonly'=>$receipt_read_only];
    ...
    $preorder_value_options = ['name'=>'jot[preorder_flag]', 'value'=>$transfer->preorder_flag,'class'=>'preorder-flag', 'disabled'=>'', 'checked' => 'checked', 'label_class'=>'receive-output', 'default'=>false];
    ...
    $received_date        = elgg_view('input/date', ['name'=>'jot[received_date]'             , 'value'=>$transfer->received_date ?: strtotime("now"), 'readonly'=>$receipt_read_only]);
    $received_date       .= elgg_view('input/hidden', ['name'=>'jot[snapshot][received_date]'      , 'value'=>$transfer->received_date]);
...
$display .= '291 $moment_value: '.$moment_value.'<br>';
Switch ($presentation){
	case 'full':
		...
		$form .= "<div class='rTable' style='width:700px'>
				<div class='rTableBody'>
					<div class='rTableRow'>
						...
						<div class='rTableCell' style='width:120px'>$moment</div>
	...
	//echo $form; goto eof;
	        	...
	        	$form .= "<div class='rTableRow' style='cursor:move'>
	        					...
	        					<div class='rTableCell' style='width:70%'>$set_properties_button $line_item
	        					    <div id='$qid_$n' class='receipt-item-properties jq-dropdown jq-dropdown-tip' style='display:none'>
 ...
 ***********************************************************/
					  ...
					  <div class='rTable pre-order'>
                         <div class='rTableBody'>
							...
							<div class='rTableRow receive-input' style='display:none;'>
								...
								<div class='rTableCell'>$received_date</div>
	...
	//echo $form; goto eof;
	        	...
	        	$form .= "<div class='rTableRow receipt_item' data-qid=$qid_n  style='cursor:move'>
	...
	case 'popup':
		...
		$vars['content']      = elgg_view('forms/transfers/elements/receipt',$body_vars); // Swings back around to pull from the presentation above.
        $vars['disable_save'] = elgg_extract('disable_save', $vars, false);
        $vars['show_title']   = elgg_extract('show_title', $vars, false);
        $vars['position']     = 'relative';
