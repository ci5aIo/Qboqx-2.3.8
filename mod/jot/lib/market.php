...
function quebx_prepare_form_vars_xxx($item = null, $parent_guid = 0) {
...
function market_prepare_brief_view_vars_xxx($item = null) {
...
function market_prepare_detailed_view_vars_xxx($item = null) {
 ...
 * @return array|false
 */
function get_item_group_collections_xxx($item_guid, $relationship, $site_guid = 0) {
	global $CONFIG;
...
function anatcasesort_xxx (&$array, $key) {
	$sorter=array();
	$ret=array();
	reset($array);
	foreach ($array as $ii => $va) {
...
function quebx_get_group_by_title_xxx($options){
    global $CONFIG;
...
function jot_render_section($options){
