...
function tasks_init() {
	...
	elgg_register_page_handler('que', 'que_page_handler');
	
	// Register a url handler
//@EDIT - 2017-09-29 - SAJ - Function call: elgg_register_entity_url_handler (deprecated since 1.9) Use the plugin hook in \ElggEntity::getURL()
...
// 	elgg_register_annotation_url_handler('task', 'tasks_revision_url');
	elgg_register_plugin_hook_handler('entity:url', 'object', 'tasks_url');

	// Register some actions
	$action_base = elgg_get_plugins_path() . 'tasks/actions/tasks';
	elgg_register_action("tasks/edit", "$action_base/edit.php");
	elgg_register_action("tasks/edit2", "$action_base/edit2.php");
	...
	$que_action_base = elgg_get_plugins_path() . 'tasks/actions/que';
	elgg_register_action("que/pick", "$que_action_base/pick.php");
	elgg_register_action("que/edit", "$que_action_base/edit.php");
	...
	//@EDIT - 2017-10-01 - SAJ - Deprecated in 1.9: register_notification_object is deprecated by elgg_register_notification_event()
	...
	elgg_register_notification_event('object', 'task', [elgg_echo('tasks:new')]);
	elgg_register_notification_event('object', 'task_top', [elgg_echo('tasks:new')]);
	...
	elgg_register_plugin_hook_handler('register', 'menu:user_hover', 'tasks_owner_block_menu');
...
function que_page_handler($task) {
	...
	elgg_push_breadcrumb(elgg_echo('tasks'). " Calendar", 'tasks/all');
	$base_dir = elgg_get_plugins_path() . 'tasks/pages/que';
	...
	switch ($task_type) {
		case 'set':
			set_input('guid'           , $task[1]);
			set_input('section'        , $task[2]);
			include "$base_dir/set.php";
			break;
		case 'pick':
			set_input('pick_type'      , $task[1]);
			set_input('item_guid'      , $task[2]);
			set_input('selection_type' , $task[3]);
			set_input('group_subtype'  , $task[4]);
			set_input('container_guid' , $task[5]);
...
/**
* Returns the URL from a task entity
*
...
* @return string
*/
function tasks_url($hook, $type, $url, $params) {
	$entity = $params['entity'];
	if (!in_array($entity->getSubtype(), ['task','task_top'])){
		// This is not a task, so there's no need to go further
		return;
	}
	return "tasks/view/$entity->guid";
}
/**
...
function tasks_entity_menu_setup($hook, $type, $return, $params) {
	...
	foreach ($return as $key => $link) {
	    ...
	    $return[$key]->setHref('tasks/edit/'. $params['entity']->guid . '/edit');
	}
