...
echo '<br>car.php<br><br>';
echo '<p><label>'.$entity->title.'</label></p>';
/**
 * 
...
// eg. characteristic name => characteristic value
// these will be parallel arrays, so $name[0] pairs with $value[0]
// these are also saved automagically
$names = $entity->component_names; // retained for backward compatibility.  
$names .= $entity->characteristic_names;
if ($names && !is_array($names)) {
	$names = array($names);
}

$values = $entity->component_values; // retained for backward compatibility.
$values .= $entity->characteristic_values;
if ($values && !is_array($values)) {
	$values = array($values);
}
echo '<label>Characteristics</label>';
	echo ':'.count($names).'<br>';
...
foreach ($names as $key => $name) {
	...
	echo '<a href="#" class="car-characteristic-remove">remove</a>';
	echo '</div>';
}
...
echo '<div class="car-component-clone">';
	echo '<div>';
	echo elgg_view('input/text', array(
		'name' => 'item[component_names][]',
		'style' => 'width: 200px;'
...
echo elgg_view('output/url', array(
	'text' => 'Add Another',
	'href' => '#',
	'class' => 'elgg-button elgg-button-action car-characteristic-clone-action' // unique class for jquery
));
echo '<br><br>';
...
<script>
	$(document).ready(function() {
		...
		// clone a node
		$('.car-characteristic-clone-action').on('click', function(e){
			e.preventDefault();
			
			// clone the node
			var html = $('.car-components-clone').html();
			
			$(html).insertBefore('.car-characteristic-clone-action');
		});
		
		// remove a node
		$('.car-characteristic-remove').on('click', function(e){
			e.preventDefault();
