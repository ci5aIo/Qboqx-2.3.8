...
// Build the location tree
	...
	$all_items = elgg_get_entities(array(
		'type' => 'object',
		'subtype' => 'market',
		'owner_guid' => $owner_guid,
		'limit' => false,
	...
	$containing_entities = elgg_get_entities_from_relationship($options);
	foreach ($containing_entities as $contained){
		$this_subtype = get_entity($contained->guid)->getSubtype();
		$these_containers[] = array('guid' => $contained->guid,
		                            'title' => $contained->title,
									'subtype'=> $this_subtype);
		$these_contents[] = elgg_get_entities_from_relationship(array(  'type' => 'object',
																		'relationship' => 'contents',
																		'relationship_guid' => $contained->guid,
																	    'inverse_relationship' => true,
																		'limit' => false,
		                                                             )
																);
		$combined_array = array_merge($these_containers, $these_contents);
		if ($this_subtype == 'market' || $this_subtype == 'item'){
		echo $contained->guid.' - '. $contained->title.'<br>';
			
		}
	}
//	echo elgg_dump($these_containers);
	echo elgg_dump($combined_array);
//echo elgg_dump($contained_items['title']);
	$containers = array();
	$depths = array();
...
echo elgg_dump($containers->title);
