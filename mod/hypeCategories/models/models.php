...
function delete_all_categories() {
	//@EDIT - 2017-10-01 - SAJ - Function call: get_entities (deprecated since 1.7) Use elgg_get_entities()
	//$objects = get_entities('object', 'category');
	$objects = elgg_get_entities(['type'=>'object', 'subtype'=>'category']);
    foreach ($objects as $object) {
        $object->delete();
    }
    return true;
...
function count_categories() {
    ...
    if ($count < get_plugin_setting('count_init', 'hypeCategories')) {
        return true;
    } else {
        return false;
    }
...
function migrate_categories() {
    ...
    if (is_array($categories) && sizeof($categories) > 0) {
        foreach ($categories as $category) {
            if ($category->sort !== $category->title) {
                $category->sort = $category->title;
            }
        }
    }
    set_plugin_setting('migrated', true, 'hypeCategories');
}
