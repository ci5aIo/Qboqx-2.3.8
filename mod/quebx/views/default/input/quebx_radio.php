...
$options2 = $vars['options2'];
unset($vars['options'], $vars['options2']);

$value = $vars['value'];
unset($vars['value']);


if ($options2 && count($options2) > 0) {
    unset($options);
	echo "<ul class=\"$class\" $id>";
	foreach ($options2 as $label => $option) {

		$vars['checked'] = in_array(elgg_strtolower($option['option']), $value);
		$vars['value'] = $option['option'];

		$attributes  = elgg_format_attributes($vars);
		if (!empty($option['id'])){$id = "id=\"{$option['id']}\"";}

		echo "<li title=\"{$option['title']}\"><input type=\"radio\" $attributes $id />{$option['label']}</li>";
	}
	echo '</ul>';
}
